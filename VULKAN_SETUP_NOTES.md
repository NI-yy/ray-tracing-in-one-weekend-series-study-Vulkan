...
- [x] CPU 版 / CUDA 版 / Vulkan 版のレンダリング時間を比較する。
...
## 17. CPU 版 / CUDA 版 / Vulkan 版のレンダリング時間を比較する

コピー元の最終レンダリング設定に合わせて、Vulkan 版でも高解像度・高サンプル数の実行確認をしました。
...
この設定に合わせるため、Vulkan 実行パスを次の値に変更しました。

```cpp
render_vulkan_material_spheres("image_vulkan_materials.ppm", 1200, 675);
...
```text
Vulkan run 1: 16.839163 sec
Vulkan run 2: 16.647813 sec
Vulkan run 3: 16.338425 sec
Vulkan run 4: 16.336989 sec
Vulkan run 5: 16.354671 sec
Vulkan run 6: 16.415986 sec
Vulkan run 7: 16.300708 sec
Vulkan run 8: 16.373922 sec
Vulkan run 9: 16.250264 sec
Vulkan run 10: 16.410027 sec
Vulkan average: 16.426797 sec
```

この時間は `Measure-Command` で測ったアプリ実行全体です。
つまり、Vulkan compute の実行時間だけではなく、PPM 書き出しも含みます。

同じ条件で CUDA 版も測ったところ、当初の CUDA 実装は `double` ベースだったためかなり遅く、10 回平均は次の結果でした。

```text
CUDA double average: 71.288225 sec
```

ただし、これは CUDA と Vulkan の API 差というより、主に数値型の差だと考えられます。
CUDA 側を `float` ベースへ変更して 1 回測り直すと、次の結果になりました。

```text
CUDA float render time: 9.05656 sec
CUDA float app total: 9.1778502 sec
CUDA float primary samples/sec: 4.4719e+07
```

このため、現時点での理解は次の通りです。

- 同じ GPU で同じような計算をする限り、CUDA と Vulkan Compute の差は API 名だけでは決まりにくい。
- 最初に Vulkan が約 4 倍速く見えた主因は、CUDA 版が `double`、Vulkan 版が `float` だったこと。
- GPU path tracer では、精度、データ配置、分岐、レジスタ使用量、occupancy、乱数処理、BVH の有無の方が支配的になりやすい。
- Vulkan の利点は、compute の速さそのものより、compute 結果をそのまま Vulkan の画像表示、post process、swapchain、将来の ray tracing pipeline へ接続しやすいこと。

README には、この Vulkan 版の最終レンダリング PNG と 10 回平均の計測結果を載せました。

## 18. 次にやること

次は、PPM 書き出しを除いた GPU compute 本体の時間を測れるようにします。

- Vulkan 側に GPU 実行時間、または少なくとも `vkQueueSubmit` から fence wait までの CPU 側計測を追加する。
- CUDA 側も kernel 実行時間と PPM 書き出し込み時間を分ける。
- そのうえで、CUDA float 版 / Vulkan float 版を同条件で比較する。
- 速度改善としては、総当たりの sphere intersection から BVH へ進む。
