...
次は簡単なマテリアル処理、または Lambertian 風の diffuse bounce に進みます。

最初はまだ CPU 版の `material` クラスを直接移植せず、shader 側で扱いやすい material id や albedo を storage buffer に追加する方針がよさそうです。
