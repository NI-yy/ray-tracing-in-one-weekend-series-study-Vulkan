...
このため、実行ごとに結果は安定しますが、1 pixel 1 sample なのでざらつきは残ります。
...
次はランダムサンプリングと複数 bounce に進みます。

想定する次の流れ:

1. 1 pixel あたり複数 sample を計算する。
2. bounce 回数を固定値で複数回にする。
3. 各 bounce で球に当たったら diffuse bounce を続ける。
4. miss したら背景色を返す。
5. sample 平均を出力する。

その後、material id や albedo を sphere buffer に追加して、球ごとに色を変える段階へ進みます。
