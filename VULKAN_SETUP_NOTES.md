...
- [ ] 球 1 個との交差判定を Compute Shader 側で行う。
...
次は CUDA 版の進め方に合わせて、球 1 個との交差判定を Vulkan Compute Shader 側へ移します。
...
5. 出力を `image_vulkan_sphere.ppm` のような別名で保存する。

ここまで進めば、CUDA 版の `GPU で球 1 個との交差判定を行う` 段階と対応づけて学習を続けられます。
